...
// 	curFrame++
// 	if curFrame == 1 {
// 		//mapp = defineWalls(mapp)
// 		//drawMinimap(mapp);
// 		//centerCam(me.m)
// 		//camFollow(me.m)
// 	}
// 	var timeLeft = math.Round((timeoutMS - (getMS() - startMS)) / 1000)
// 	//fmt.Println("timeLeft:", timeLeft)
// 	//menu_mc.timeLeftBox.text = formatSeconds(timeLeft,"seconds");
// 	if timeLeft <= 0 {
// 		sendToSocket("race_over`")
// 	}
// 	if !me.finished && !me.forfeit {
...
// 	}
// 	for _, guy := range guys {
// 		if guy.finished == false && guy.forfeit == false {
// 			guy.move()
// 		}
// 	}
// 	updateCounter--
// 	if (updateCounter <= 0 || forceMyPositionUpdate) && me.finished == false && me.forfeit == false {
...
// 	// mines
// 	tmp1 := placedMines[:0]
// 	for _, m := range placedMines {
// 		if m.placeTimer == 1 {
// 			fmt.Println("placing mine", m)
// 			course[int(m.x/30)][int(m.y/30)] = &block{x: m.x, y: m.y, t: blockMine}
// 		} else {
// 			m.placeTimer--
// 			tmp1 = append(tmp1, m)
// 		}
...
// 	}
// 	for i := len(tmp1); i < len(placedMines); i++ {
// 		placedMines[i] = nil
// 	}
// 	placedMines = tmp1
...
// 		bx := int(math.Round(l.x / segSize))
// 		by := int(math.Round(l.y / segSize))
// 		if bx > -1 && bx < len(course) && by > -1 && by < len(course[0]) {
// 			var _loc5 = course[bx][by]
// 			if _loc5 != nil {
// 				if _loc5.t == blockBrick || _loc5.t == blockMine {
// 					course[bx][by] = nil
// 					continue
